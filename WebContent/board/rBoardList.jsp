<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    String sqlTotal = "select count(*) cnt from reboard";
    ...
    String sql = " select "; 
    	  ...
    	  // sql+= "         left(rdate,10) rdate, ";   //날짜만 출력하기 시간x   sql에서 해결하는게 더 빠름 
    	   ...
    	   sql+= " from ";
		   sql+= "         reboard "; 
		   sql+= " order by fid desc, thread asc LIMIT "+startNo+","+unit; // 0번 index부터 10개 출력!  fid로 역순정렬하고 thread로 순차적정렬 
...
<html lang="ko">
  ...
  <body>
    ...
    <div id="jb-container">  <!-- container 전체를 감쌈-->
      ...
      <input type="button" value="글쓰기" onClick="location='rBoardWrite.jsp'">  <!-- onClick= 자바스크립 클릭시 이동 location='목적지' -->
      </div>
      	...
      	<table class="table1">
      		...
      		%>
      			<tr>
      				...
      				<%
      				//a원글 / aa댓글 /댓글을 공백으로 한칸씩 밀어냄 
      				for(int i=1; i<thread.length(); i++){    //제목 출력되기전에 공백을 넣음  a/ aa/ aaa 등등 
      					out.print("&nbsp;&nbsp;&nbsp;");
      				}
      				if( thread.length() > 1 ) out.print("[답글]");
      				%>
      				 
      				<a href="rBoardDetail.jsp?unq=<%=unq%>"><%=title %></a></td>  <!-- 타이틀에 unq값으로 상세보기로 넘어가기 -->
      ...
      %>
      	<a href="rBoardList.jsp?pageNo=<%=i %>"><%=i %></a>
      <% 	
      	}
      %>
